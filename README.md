...
3. Fill in:
   - **JMAP Server URL**: `https://jmap.example.com` (base URL only - endpoints auto-discovered)
   - **Email**: `user@example.com`
   - **Password**: Your password

**Use case:** Development, testing, or servers without OAuth2 support.

**Note:** The node automatically discovers JMAP endpoints via `/.well-known/jmap` (RFC 8620). Just provide the base server URL.
...
#### Fastmail Configuration (Bearer Token)

Fastmail uses a specific JMAP endpoint structure. Use Bearer Token authentication:

```
JMAP Server URL: https://api.fastmail.com
Authentication Method: Bearer Token
Access Token: <Your Fastmail API token>
```
...
3. Create a new token with scope: `urn:ietf:params:jmap:mail`
4. Copy the token to your n8n credential

**Note:** Fastmail's JMAP endpoints are auto-discovered. You only need to provide the base URL `https://api.fastmail.com`.
...
The JMAP Server URL configuration has changed. You should now provide only the **base URL** of your JMAP server. The node will automatically discover the session and API endpoints via `/.well-known/jmap` (per RFC 8620).

**Before:** `https://jmap.example.com/jmap`
**After:** `https://jmap.example.com`

**What changed:**

- The node now performs automatic endpoint discovery
- Supports servers with non-standard endpoint structures (like Fastmail)
- No need to guess the correct API path

**Backwards Compatibility:**

- Existing workflows will continue to work
- The discovery mechanism falls back to common endpoint patterns
- You can optionally update your credentials to use base URLs only

### Server-Specific URL Examples

| Server       | JMAP Server URL             |
| ------------ | --------------------------- |
| Apache James | `https://james.example.com` |
| Fastmail     | `https://api.fastmail.com`  |
| Stalwart     | `https://mail.example.com`  |
| Cyrus IMAP   | `https://cyrus.example.com` |
